...
def getTrainer(language: str):
    try:
        asr_model = mo.getASRModel(language, use_whisper=True, attn_implementation="eager")
    except TypeError:
...
class PronunciationTrainer:
    ...
    def __init__(self, asr_model: mi.IASRModel, word_to_ipa_converter: mi.ITextToPhonemModel) -> None:
    ...
    def getTranscriptAndWordsLocations(self, audio_length_in_samples: int):
        ...
        word_locations_in_samples = [(int(np.maximum(0, word['start_ts'] - fade_duration_in_samples)), 
    ...
    def getWordsRelativeIntonation(self, Audio: torch.tensor, word_locations: list):
        ...
        for word in range(len(word_locations)):
            ...
            intonations[word] = torch.sqrt(torch.mean(Audio[0][intonation_start:intonation_end]**2))
    ...
    def getAudioTranscript(self, recordedAudio: torch.Tensor = None):
        ...
        if current_recorded_audio.dim() == 1:
            current_recorded_audio = current_recorded_audio.unsqueeze(0)
        if current_recorded_audio.shape[0] != 1:
            current_recorded_audio = current_recorded_audio[0:1, :]
