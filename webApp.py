...
#         return jsonify(json.loads(body)), 200, cors_headers
#     except Exception as e:
#         print(f'Error in /getSample: {str(e)}')
#         return jsonify({'error': str(e)}), 500, cors_headers
...
#     try:
#         event = {'body': json.dumps(request.get_json(force=True))}
#         response = lambdaSpeechToScore.lambda_handler(event, [])
#         body = response.get('body', '{}')
#         if not body.strip():
#             return jsonify({'error': 'Empty response from audio processing'}), 400, cors_headers
#         return jsonify(json.loads(body)), 200, cors_headers
#     except Exception as e:
#         print(f'Error in /GetAccuracyFromRecordedAudio: {str(e)}')
#         return jsonify({'error': str(e)}), 500, cors_headers
...
#     try:
#         data = request.get_json(force=True)
#         print(f"Request body: {data}")  # Debug: Log the request body
...
#             # Use None for memory to avoid loading history
#             suggestion_prompt = (
...
#                     k=5
#                 )
#             # Seed memory with the current ai_response
...
#         # Kiểm tra và điều chỉnh phản hồi
...
try:
    ...
    data_word_en = pd.DataFrame()  # Fallback to empty DataFrame
except Exception as e:
...
def get_next():
    try:
        ...
        mode = data.get('mode', 'sentence')  # Default to sentence mode

        if language != "en" or mode != "word_pronunciation":
            # Fall back to lambdaGetSample for non-word-pronunciation requests
            response = lambdaGetSample.lambda_handler(event, [])
            if isinstance(response, dict):
                body = response.get('body', '{}')
            else:
        ...
        # Word pronunciation mode for English - use existing ID from CSV
        ...
        sample_id = str(sample["id"])  # Use existing ID from CSV
        word = sample["word"]
        ipa = sample["ipa"]
        ...
        response = {
            ...
            "language": language,
            "mode": mode,
            "text": word  # Use word as the text for consistency
        }
...
def get_accuracy_from_recorded_audio():
    ...
    try:
        ...
        recorded_audio = request_data.get("recorded_audio")
        word = request_data.get("word")  # Nhận word thay vì sample_id
        
        if not recorded_audio or not word:
            return _corsify_actual_response(
                jsonify({"error": "Missing recorded_audio or word"})
            ), 400
        
        # Tra cứu sample_id từ word trong data_word_en.csv
        word_info = data_word_en[data_word_en['word'] == word]
        if word_info.empty:
            return _corsify_actual_response(
                jsonify({"error": f"Word {word} not found in dataset"})
            ), 404
        
        sample_id = str(word_info.iloc[0]["id"])  # Lấy sample_id từ word
